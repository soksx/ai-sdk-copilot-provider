...
export class CopilotResponsesLanguageModel implements LanguageModelV3 {
	...
	private async getArgs(options: LanguageModelV3CallOptions) {
		...
		const include: CopilotResponsesIncludeOptions = copilotOptions?.include;
		...
		const baseArgs = {
			...
			...(modelConfig.isReasoningModel &&
				(copilotOptions?.reasoningEffort != null || copilotOptions?.reasoningSummary != null) && {
					reasoning: {
						...(copilotOptions?.reasoningEffort != null && {
							effort: copilotOptions.reasoningEffort,
						}),
						...(copilotOptions?.reasoningSummary != null && {
							summary: copilotOptions.reasoningSummary,
						}),
					},
				}),
	...
	async doStream(
		...
		return {
			stream: response.pipeThrough(
				new TransformStream<
					ParseResult<z.infer<typeof copilotResponsesChunkSchema>>,
					...
					transform(chunk, controller) {
						...
						if (isResponseOutputItemAddedChunk(value)) {
							...
							finishReason = mapCopilotResponseFinishReason({
								...
								hasFunctionCall,
								isJsonResponseFromTool: options.responseFormat?.type === "json" && hasFunctionCall,
							});
...
function isTextDeltaChunk(
	chunk: z.infer<typeof copilotResponsesChunkSchema>
